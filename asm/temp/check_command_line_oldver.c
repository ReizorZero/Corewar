...
void	write_all_tokens(t_asm *the_asm, t_line **line)
{
	...
	while (s[i] != '\0' && s[i + 1] != '\0')
	{
		if (s[i] == SEPARATOR_CHAR && s[i + 1] == SEPARATOR_CHAR)
		{
			printf("Error! Several separators in a row.\n");
		}
		i++;
	}
	...
	while (s[i] != '\0' && (s[i] == ' ' || s[i] == '\t'))
		i++;
	if (s[i] != '\0')
		printf("junk shit at the end of the line\n");
}
...
int		separate_tokens(t_asm *the_asm, int t_i)//, t_line **line)
{
	...
	if (seps > 2)
	{
		printf("Error! Too many separators.\n");
		exit (0);
	}
	//CHECK FUCKED UP CASES LIKE r1,, AND SO ONE -- DONE!
